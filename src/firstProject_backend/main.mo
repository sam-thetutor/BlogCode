...
switch(postStore.get(_postID)) {
  ...
  case(?post) {
    ...
    let tempData = Buffer.fromArray<Types.Comment>(post.comments);
    tempData.add({
    ...
    postStore.put(_postID,{post with comments = Buffer.toArray<Types.Comment>(tempData)});
...
switch(postStore.get(_postID)) {
  case(?post) { 
    ...
    if(caller != post.userId){
      ...
      let tempData = Buffer.fromArray<Types.Comment>(post.comments);
       tempData.filterEntries(func(_, x) = x.commentId != _commentID);
       let arrayData = Buffer.toArray<Types.Comment>(tempData);
       postStore.put(_postID,{post with comments =arrayData });
       return #ok();
   };
...
switch(postStore.get(_postID)) {
  case(null) { return []  };
  case(?posts) { return posts.comments };
};
...
switch(postStore.get(_postID)) {
  ...
  case(?post) {
    ...
    if(caller != post.userId){
      ...
      let newBuf = Buffer.mapEntries<Nat, Types.Comment>(tempData, func (x, post) { 

        if(post.commentId == _commentID){
